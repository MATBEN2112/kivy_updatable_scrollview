...
class LoadSignContainer(FloatLayout):
    def __init__(self, scroll_view, hide_animation, _min_overscroll, y):
        super(LoadSignContainer, self).__init__()
        self.scroll_view = scroll_view
        self._min_overscroll = _min_overscroll
        self.hide_animation = hide_animation
        # set container pos and size according to scrollview
        self.ids.refresh_container_inner.size =(self.scroll_view.size[0],abs(self._min_overscroll))
        self.ids.refresh_container_inner.pos = (self.scroll_view.x,self.scroll_view.y+y)
        # create instance of load sign
        self.load_sign = self.scroll_view.load_sign_up(self.scroll_view,size_hint = (None, None))
        # set load sing pos and size according to scrollview
        self.load_sign.height = abs(self._min_overscroll)
        self.load_sign.pos = (
            self.ids.refresh_container_inner.x+(self.scroll_view.width-self.load_sign.width)/2,
    ...
    def move_load_sign(self, dy):
        # fading effect
        if self.scroll_view.fading_effect:
...
class LoadSignContainerUp(LoadSignContainer):
    def __init__(self, scroll_view, **kwargs):
        _min_overscroll = scroll_view.min_overscroll_up
        hide_animation = scroll_view.hide_animation_up
        super(LoadSignContainerUp, self).__init__(scroll_view,hide_animation,_min_overscroll,0)
        y = scroll_view.height-self.ids.refresh_container_inner.height
        super().__init__(scroll_view,hide_animation,_min_overscroll,y)
            
class LoadSignContainerDown(LoadSignContainer):
    def __init__(self, scroll_view, **kwargs):
        _min_overscroll = scroll_view.min_overscroll_down
        hide_animation = scroll_view.hide_animation_down
        super().__init__(scroll_view,hide_animation,_min_overscroll,0)
...
class UpdatableScrollViewApp(App):
    ...
    async def load_more_callback(self, count = 10):
        await asyncio.sleep(2)
